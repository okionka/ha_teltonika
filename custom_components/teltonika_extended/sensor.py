"""Sensor platform for Teltonika Extended — all sensors mirroring the Modbus YAML."""
...
SYSTEM_SENSORS: tuple[TeltonikaSensorDesc, ...] = (
    TeltonikaSensorDesc(key="uptime", name="System uptime",
        icon="mdi:timer-outline", native_unit_of_measurement=UnitOfTime.SECONDS,
        state_class=SensorStateClass.TOTAL_INCREASING, group="system",
        value_fn=lambda d: _a(d, "static", "board") and None or None,
    ),
    # Note: uptime is not in DeviceStatusData — dedicated note in README
    TeltonikaSensorDesc(key="hostname", name="Hostname",
        icon="mdi:router-network", group="system",
        value_fn=lambda d: _a(d, "static", "hostname"),
    ),
    TeltonikaSensorDesc(key="firmware", name="Firmware version",
        icon="mdi:chip", group="system",
        value_fn=lambda d: _a(d, "static", "fw_version"),
    ),
    TeltonikaSensorDesc(key="model", name="Device model",
        icon="mdi:router-wireless", group="system",
        value_fn=lambda d: _a(d, "static", "model"),
    ),
    TeltonikaSensorDesc(key="serial", name="Serial number",
        icon="mdi:barcode", group="system",
        value_fn=lambda d: _a(d, "mnf_info", "serial"),
    ),
    TeltonikaSensorDesc(key="mac", name="LAN MAC address",
        icon="mdi:ethernet", group="system",
        value_fn=lambda d: _a(d, "mnf_info", "mac"),
    ),
    TeltonikaSensorDesc(key="device_name", name="Device name",
...
GPS_SENSORS: tuple[TeltonikaSensorDesc, ...] = (
    TeltonikaSensorDesc(key="latitude", name="GPS latitude",
...
WAN_SENSORS: tuple[TeltonikaSensorDesc, ...] = (
    TeltonikaSensorDesc(key="wan_ip", name="WAN IP address",
        icon="mdi:ip-network", group="wan",
        value_fn=lambda d: _a(d, "ip_address"),
    ),
    TeltonikaSensorDesc(key="wan_type", name="WAN type",
        icon="mdi:network-outline", group="wan",
        value_fn=lambda d: _a(d, "wan_type"),
    ),
    TeltonikaSensorDesc(key="wan_iface", name="WAN interface",
...
def _usage_sensors(sim: int) -> tuple[TeltonikaSensorDesc, ...]:
    ...
    def _s(key: str, label: str, icon: str, attr: str) -> TeltonikaSensorDesc:
    ...
    return (
        _s("rx_today",      "received today",       "mdi:download",  "rx_today"),
        _s("tx_today",      "sent today",            "mdi:upload",    "tx_today"),
        _s("rx_24h",        "received last 24 h",   "mdi:download",  "rx_last_24h"),
        _s("tx_24h",        "sent last 24 h",        "mdi:upload",    "tx_last_24h"),
        _s("rx_week",       "received this week",   "mdi:download",  "rx_week"),
        _s("tx_week",       "sent this week",        "mdi:upload",    "tx_week"),
        _s("rx_last_7d",    "received last 7 days", "mdi:download",  "rx_last_7d"),
        _s("tx_last_7d",    "sent last 7 days",      "mdi:upload",    "tx_last_7d"),
        _s("rx_month",      "received this month",  "mdi:download",  "rx_month"),
        _s("tx_month",      "sent this month",       "mdi:upload",    "tx_month"),
        _s("rx_last_30d",   "received last 30 days","mdi:download",  "rx_last_30d"),
        _s("tx_last_30d",   "sent last 30 days",     "mdi:upload",    "tx_last_30d"),
        _s("rx_last_month", "received last month",  "mdi:download",  "rx_last_month"),
        _s("tx_last_month", "sent last month",       "mdi:upload",    "tx_last_month"),
        _s("rx_last_week",  "received last week",   "mdi:download",  "rx_last_week"),
        _s("tx_last_week",  "sent last week",        "mdi:upload",    "tx_last_week"),
    )
...
class _Base(CoordinatorEntity[TeltonikaCoordinator], SensorEntity):
    ...
    def __init__(self, coordinator: TeltonikaCoordinator, entry: ConfigEntry,
                 description: TeltonikaSensorDesc) -> None:
